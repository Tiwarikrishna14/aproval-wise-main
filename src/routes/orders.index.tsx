...
const TABS = ["All Orders", "Draft", "Submitted", "Under Review", "Approved", "In Transit", "Delivered", "Rejected", "Cancelled"];
        ...
        <Tabs defaultValue="All Orders">
          <div className="border-b border-border px-3 pt-2">
            <TabsList className="h-10 bg-transparent p-0 gap-1 flex-wrap">
              {TABS.map((t) => (
                <TabsTrigger key={t} value={t} className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary data-[state=active]:shadow-none h-8 rounded-md text-[13px] font-medium text-muted-foreground">
                  {t}
                </TabsTrigger>
              ))}
            </TabsList>
          ...
          <div className="grid gap-3 border-b border-border p-4 md:grid-cols-[1fr_auto_auto_auto]">
            <div className="relative">
              ...
              <input className="h-9 w-full rounded-md border border-border bg-background pl-9 pr-3 text-sm outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/15" placeholder="Search by order number, product, branch…" />
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-surface text-xs uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="w-10 pl-4"><input type="checkbox" className="rounded border-border" /></th>
              ...
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id} className="border-t border-border hover:bg-surface/50">
                    <td className="pl-4"><input type="checkbox" className="rounded border-border" /></td>
                    ...
                    <td className="px-4 py-3 text-right tabular-nums font-medium">₹{o.totalAmount.toLocaleString("en-IN")}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <Button size="sm" variant="ghost" asChild><Link to="/orders/$id" params={{ id: o.id }}>Open</Link></Button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3 text-xs text-muted-foreground">
            <div>Showing 1–5 of 48 orders</div>
